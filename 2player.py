...
end_font = pygame.font.SysFont('tahoma', 150)
...
def pause():
    bg_music.stop()
    global cx, cy, r, playerR_x, playerR_y, playerR_w, playerR_h, botR_h, botR_w, botR_x, botR_y, player_score, bot_score
    player_score = 0
    bot_score = 0
...
# Game loop
def game():
    global playerR_y, rect1_vel, rect2_vel, player_score, bot_score, botR_y, start_ticks
    start_ticks = pygame.time.get_ticks()  # Timer start
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_w:  # go up
                    rect2_vel = -2
                elif event.key == pygame.K_s:  # go down
                    rect2_vel = 2
